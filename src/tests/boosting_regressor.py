...
def transform_train_df(dataset_path, train_date, target_col, date_col, is_plot_pca_threshold, is_plot_model_selection, use_pca, use_normalize):
    ...
    df.drop(['AwayTeam_GF', 'HomeTeam_GF', 'Season', 'Round', 'HomeTeam', 'AwayTeam', 'H2H_Away_Goals','H2H_Away_Wins',
             'H2H_Draws',  'H2H_Home_Goals',  'H2H_Home_Wins',  'H2H_Total_Matches', 'HomeTeam_Result'], axis=1, inplace=True)
...
def boosting_regressor():
    # Import config plot
    with open('D:/HUST/_Intro to DS/Capstone Project/Football-Match-Prediction/config_regressor.yaml', 'r') as file:
...
def get_regressor_input(df_team, df_opponent, team_name, opponent_name, target_time):
...
x_test = get_regressor_input(df_team, df_opponent, 'Barcelona', 'Real Madrid', '2024-08-22')
