...
def get_1_sample_classifier():
    ...
    x_train, x_test, y_train, y_test = transform_train_df_classifier(
...
def main():
    Classifier_Model_Setting(TARGET_COL='HomeTeam_Result', MODEL='DecisionTreeClassifier', config_path='D:/HUST/_Intro to DS/Capstone Project/Football-Match-Prediction/config.yaml')
    x_train_classier = get_1_sample_classifier()
    classifier_model = boosting_classifier()
    #x_train = extract_input(date,home,away)
    game_result = classifier_model.predict(x_train_classier)
    print(game_result)
